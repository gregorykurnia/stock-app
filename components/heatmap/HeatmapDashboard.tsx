...
import type { SectorAggregate } from "@/lib/heatmap/calculations";
...
type SectorSortKey = "sector" | "capWeightedReturn" | "equalWeightedReturn" | "pctAdvancing" | "strongest";
type SortDirection = "asc" | "desc";

function sectorSortValue(sector: SectorAggregate, key: SectorSortKey): number | string | null {
  if (key === "sector") return sector.sector;
  if (key === "strongest") return sector.strongest?.symbol ?? null;
  return sector[key];
}

function sortSectorAggregates(sectors: SectorAggregate[], key: SectorSortKey, direction: SortDirection): SectorAggregate[] {
  return [...sectors].sort((a, b) => {
    const av = sectorSortValue(a, key);
    const bv = sectorSortValue(b, key);
    ...
    return (comparison || a.sector.localeCompare(b.sector)) * (direction === "asc" ? 1 : -1);
  });
}
...
export default function HeatmapDashboard() {
  ...
  const [data, setData] = useState<HeatmapResponse | null>(null); const [error, setError] = useState(""); const [loading, setLoading] = useState(true); const [refreshing, setRefreshing] = useState(false); const [selected, setSelected] = useState<HeatmapStock | null>(null); const [sectorSort, setSectorSort] = useState<{ key: SectorSortKey; direction: SortDirection }>({ key: "sector", direction: "asc" });
  ...
  const sortedSectors = useMemo(() => sortSectorAggregates(sectors, sectorSort.key, sectorSort.direction), [sectors, sectorSort]);
  const leaders = useMemo(() => rankBy(visible, (stock) => periodReturn(stock.returns, state.period), { limit: 5 }), [visible, state.period]);
  const toggleSectorSort = (key: SectorSortKey) => setSectorSort((current) => current.key === key ? { key, direction: current.direction === "asc" ? "desc" : "asc" } : { key, direction: "asc" });
  const ariaSort = (key: SectorSortKey) => sectorSort.key === key ? (sectorSort.direction === "asc" ? "ascending" : "descending") : "none";
  const sectorSortButton = (key: SectorSortKey, label: string) => <button type="button" className="inline-flex items-center gap-1 font-semibold hover:text-gray-700" onClick={() => toggleSectorSort(key)}>{label}<span aria-hidden="true" className="text-[10px] text-gray-400">{sectorSort.key === key ? (sectorSort.direction === "asc" ? "↑" : "↓") : "↕"}</span></button>;
