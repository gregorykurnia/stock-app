...
test("deposits increase pocket and total cash as external flow", () => {
  const state = reducePortfolioLedger([
    transaction("deposit", { bucket: "index", cashDelta: 2_000, externalFlow: 2_000 }),
  ]);

  assert.equal(state.buckets.index.cash.USD, 2_000);
  assert.equal(state.total.cash.USD, 2_000);
  assert.equal(state.buckets.index.externalFlow.USD, 2_000);
...
test("buying with cash preserves equity while creating weighted-average cost basis", () => {
  const state = reducePortfolioLedger([
    transaction("deposit", { bucket: "index", cashDelta: 4_000, externalFlow: 4_000, occurredAt: "2026-09-17T09:00:00.000Z" }),
    transaction("buy", {
      bucket: "index",
      ticker: "ABC",
      quantity: 20,
  ...
  assert.equal(state.buckets.index.cash.USD, 1_100);
  assert.equal(state.buckets.index.positions.ABC.quantity, 30);
  assert.equal(state.buckets.index.positions.ABC.costBasisUsd, 2_900);
  assert.equal(state.buckets.index.positions.ABC.averageCostUsd, 2_900 / 30);
  assert.equal(state.buckets.index.realizedGainUsd, 0);
});
...
test("position transfers preserve weighted-average cost basis", () => {
  ...
  assert.equal(state.buckets.index.positions.ABC.quantity, 4);
  assert.equal(state.buckets.index.positions.ABC.costBasisUsd, 400);
  assert.equal(state.total.realizedGainUsd, 0);
});

test("Treasury supports receiving a position transfer from Index", () => {
  const state = reducePortfolioLedger([
    transaction("opening_balance", { bucket: "index", ticker: "T-BILL", quantity: 4, price: 100 }),
    transaction("transfer", {
  ...
  assert.equal(state.buckets.index.positions["T-BILL"].quantity, 2);
...
test("Treasury cash transfers preserve total cash", () => {
  const state = reducePortfolioLedger([
    transaction("opening_balance", { bucket: "index", cashDelta: 2_000 }),
    transaction("transfer", {
  ...
  assert.equal(state.buckets.index.cash.USD, 882);
...
test("same-day activity is applied in timestamp order", () => {
  const state = reducePortfolioLedger([
    transaction("buy", {
      bucket: "index", ticker: "ABC", quantity: 1, price: 100, grossAmount: 100, cashDelta: -100,
      occurredAt: "2026-09-17T11:00:00.000Z",
    }),
    transaction("deposit", {
      bucket: "index", cashDelta: 100, externalFlow: 100,
      occurredAt: "2026-09-17T10:00:00.000Z",
    }),
  ]);

  assert.equal(state.buckets.index.cash.USD, 0);
  assert.equal(state.buckets.index.positions.ABC.quantity, 1);
});
...
test("invalid transactions and duplicate ids are rejected", () => {
  ...
  const deposit = transaction("deposit", { bucket: "index", cashDelta: 100, externalFlow: 100 });
...
test("append planning treats identical retries as no-ops and rejects conflicting ids", () => {
  const opening = transaction("opening_balance", { bucket: "index", cashDelta: 500 });
  const deposit = transaction("deposit", { bucket: "index", cashDelta: 100, externalFlow: 100 });
  ...
  assert.equal(reducePortfolioLedger(plan.transactions).buckets.index.cash.USD, 600);
...
test("removal planning deletes a manual activity only when the remaining ledger replays", () => {
  const opening = transaction("opening_balance", { bucket: "index", ticker: "ABC", quantity: 10, price: 100 });
  const cash = transaction("opening_balance", { bucket: "index", cashDelta: 1_000 });
  const sell = transaction("sell", {
    bucket: "index", ticker: "ABC", quantity: 2, price: 110, grossAmount: 220, cashDelta: 220,
  });
  ...
  assert.equal(reducePortfolioLedger(plan.transactions).buckets.index.positions.ABC.quantity, 10);
});

test("removal planning protects opening balances and reconciliation records", () => {
  const opening = transaction("opening_balance", { bucket: "index", cashDelta: 1_000 });
